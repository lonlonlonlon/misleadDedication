...
class BrotLineTaskWide implements Worker\Task
{
    ...
    static function getChar(int $x, int $y, int $time, int $width, int $height, int $endTime, int $iterations) {
        ...
        if ($time > 0) {
            $brotVal = self::checkFreakout(($normX * 5) - 2, ($normY * 3) - 1.5,  $iter, 2, $normTime * $normX * 3 - 0.5, $normTime * $normY * 3 + 0.5);
        } else {
            $brotVal = self::checkFreakout(($normX * 5) - 2, ($normY * 3) - 1.5,  $iter, 2, $normTime * $normY * 3 - 0.5, $normTime * $normX * 3 + 0.5);
        }
